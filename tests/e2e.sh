...
jq -e 'type == "object" and length == 11' "$manifest" >/dev/null
...
cleanup() {
  ...
  for secret in \
    ...
    docs-mcp-openai-api-key \
    docs-mcp-posthog-api-key \
  ...
  [[ -z ${anythingllm_state_file:-} ]] || rm -f -- "$anythingllm_state_file"
...
anythingllm_state_file="$HOME/.local/state/homelab/anythingllm/.env"
[[ -e $anythingllm_state_file || -L $anythingllm_state_file ]] && \
  die "$anythingllm_state_file already exists; refusing to overwrite an existing state file"
install -d -m 0700 "$(dirname -- "$anythingllm_state_file")"
touch "$anythingllm_state_file"
chmod 0666 "$anythingllm_state_file"
...
supernote:
  ...
  redis_password: e2e-supernote-redis-password
vaultwarden:
