...
| `changedetection` | Rootless single service | Changedetection edge network; `changedetection` UI route; Internet egress | Durable data volume (`homelab-changedetection-data`); non-root `1000:1000`; in-app password protection; zero capabilities; `RunInit=true`; `ReadOnly=true` |
